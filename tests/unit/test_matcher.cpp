...
TEST(LimitOrder, OpenResult_AddsOrderToBook) {
    OrderBook ob;
    ob.matchLimitOrder(makeOrder(1, 100, 10, Side::BID));
...
TEST(LimitOrder, PartialFill_RemainderPreservesOrderType) {
    OrderBook ob;
    ob.add(makeOrder(1, 100, 5, Side::ASK));
    // Submit a GoodForDay limit that partially fills.
    ob.matchLimitOrder(makeOrder(2, 100, 10, Side::BID, OrderType::GoodForDay));

    // The remainder should still be GoodForDay - prune should remove it.
